...
function ProvinceNutrients() {
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL+'/api/source', {
                method: 'POST',
                headers: {
    ...
    const handleSubmitUpdate = async (values) => {
        ...
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL+`/api/updatesource/${currentRecord._id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const fetchSources = async () => {
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL+'/api/getsource', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: 'image',
            key: 'image',
            render: (text) => text ? <img src={process.env.REACT_APP_API_BASE_URL+`/api/images/${text}`} alt="source" style={{ width: 50, height: 50 }} /> : null,
        },
        {
            title: 'Actions',
            key: 'actions',
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL+`/api/source/${id}`, {
                method: 'DELETE',
            });
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        {currentRecord?.image && (
                            <img
                                src={process.env.REACT_APP_API_BASE_URL+`/api/images/${currentRecord.image}`}
                                alt="current"
                                style={{ width: 100, marginTop: 10 }}
                            />
                        )}
